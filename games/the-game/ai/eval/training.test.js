...
test('a smoke-sized run is never committed', function () {
    ...
    // snapshot name was the wrong half-measure: anything reading snapshots by
    // filename would fold a population-32 run in among the real searches, and
    // the repo sat dirty after every smoke run.
    ...
    assert.ok(/\.smoke\.json/.test(guard.slice(0, 900)),
        'a smoke snapshot keeps the real snapshot name, so it reads as a result ' +
        'and the repo stays dirty after every smoke run');
});
