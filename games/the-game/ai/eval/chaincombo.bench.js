...
console.log('switches: depth ' + loaded.switches.depth + '  beam ' + loaded.switches.beam +
            ...
            '  modes ' + (loaded.switches.modes
                ? 'ON (' + (loaded.switches.goal || 'no goal') +
                  (loaded.switches.buildToward ? ' / toward ' + loaded.switches.buildToward : '') +
                  ' / stopFloor ' + loaded.switches.stopFloor + ')'
                : 'off'));
...
var modeTotals = { BUILD: 0, ATTACK: 0, FORCED: 0 }, broken = 0, payless = 0, riseUn = 0;
var perGame = [];
SCENARIOS.forEach(function (sc) {
    for (var seed = 1; seed <= GAMES; seed++) {
        var r = bench.run(loaded.weights, seed, {
            ...
            goal: loaded.switches.goal, alsoTake: loaded.switches.alsoTake,
            buildToward: loaded.switches.buildToward
        });
        ...
        modeTotals.ATTACK += r.modeCounts.ATTACK;
